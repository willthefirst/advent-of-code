...
const applyFloatingBitmask = (bitmask: string, initial32BitValue: string): string => {
	let floatingBit = "";
...
const initializeWithMemAddressDecoder = (instructions: string[]): { [key: number]: string } => {
	let mem: { [key: number]: string } = {};
	let currentMask = "";
	...
	instructions.forEach((line) => {
		if (line.includes("mask")) {
			...
			let floatingBitmask: string = applyFloatingBitmask(currentMask, bitAddress);
			let addresses: number[] = getAddressesFromFloatingBitmask(floatingBitmask);
