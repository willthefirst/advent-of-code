...
const getVisibleSeatAlongVector = (
	...
	if (nextX < 0 || nextX > seats[0].length - 1 || nextY < 0 || nextY > seats.length - 1) {
		return 0;
	}
...
const nextSeatState = (state: string, numOccupied: number, numOccupiedLimit: number): string => {
...
const seatGuests = (
	...
	const newSeats = updateSeatsByCountMethod(oldSeats, countMethod, numOccupiedLimit);
