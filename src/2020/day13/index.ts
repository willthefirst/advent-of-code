...
const getSoonestDepartTimeAfter = (
  earliestTime: number,
  busId: number
): number => {
...
const getSoonestDepartTimeBefore = (
  earliestTime: number,
  busId: number
): number => {
...
const solvePart1 = async () => {
  ...
  busIds.forEach((busId) => {
    const soonestDepartTime = getSoonestDepartTimeAfter(
      earliestDepartTime,
      busId
    );
...
const earliestTimeForBusEveryMinute = (
  busIds: busId[],
  times: busId[]
): number => {
  ...
  for (let i = 0; i < times.length; i++) {
    ...
    if (differenceFromFirst === 0) {
      ...
      times[0] = getSoonestDepartTimeBefore(
        (times[i] as number) - i,
        firstBusId
      );
      return earliestTimeForBusEveryMinute(busIds, times);
...
/* Part 2 */
solvePart2().then((result) =>
  console.log('Part 2 solution is', result === 1068781)
);
